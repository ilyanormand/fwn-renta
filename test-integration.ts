...
async function main() {
    ...
    try {
        if (statSync(EXPECTED_TOTALS_FILE).isFile()) {
            ...
            console.log(`ℹ️  Loaded ${Object.keys(expectedTotals).length} expected totals.`);
        }
    } catch (e) {
        console.warn("⚠️  Could not load expected_totals.json");
    }

    const vendorResults: Record<string, { file: string; status: "✅" | "⚠️" | "❌"; details: string }[]> = {};
        ...
        for (const pdfFile of files) {
            ...
            try {
                ...
                if (result.success && result.data) {
                    ...
                    vendorResults[vendorName].push({ file: pdfFile, status: "❌", details: result.error || "Unknown error" });
                    results.failed++;
                }
            } catch (error: any) {
                const expected = expectedTotals[pdfFile] !== undefined ? expectedTotals[pdfFile] : "N/A";
                ...
                vendorResults[vendorName].push({ file: pdfFile, status: "❌", details: `Exception: ${error.message}` });
                results.failed++;
            }
        }
    }
    ...
    console.log(`Passed (Perfect):      ${results.passed - Object.values(vendorResults).flat().filter(t => t.status === "⚠️").length}`);
    console.log(`Passed (Warnings):     ${Object.values(vendorResults).flat().filter(t => t.status === "⚠️").length}`);
