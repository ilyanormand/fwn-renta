...
async function testNutrimeoIntegration() {
  ...
  const pdfPath = path.join(process.cwd(), "Nutrimeo.pdf");
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(`📊 Extracted ${result.data?.lineItems?.length || 0} line items`);
      console.log(`💰 Total amount: ${result.data?.invoiceMetadata?.total}`);
      console.log(`📋 Invoice number: ${result.data?.invoiceMetadata?.invoiceNumber}`);
      ...
      if (result.data?.lineItems && result.data.lineItems.length > 0) {
        ...
        result.data.lineItems.slice(0, 3).forEach((item, index) => {
          console.log(`  ${index + 1}. ${item.supplierSku} - ${item.description}`);
          console.log(`     Qty: ${item.quantity}, Unit Price: ${item.unitPrice}, Total: ${item.total}`);
        });
      }
