...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (error) {
    ...
    return redirect(
      "/app/google-api?error=" +
        encodeURIComponent(`OAuth2 authorization failed: ${error}`)
    );
  }
  ...
  if (state !== "google-sheets-auth") {
    return redirect(
      "/app/google-api?error=" + encodeURIComponent("Invalid state parameter")
    );
  }
  ...
  if (!settings.oauth2Config) {
    return redirect(
      "/app/google-api?error=" + encodeURIComponent("OAuth2 not configured")
    );
  }
  ...
  try {
    ...
    return redirect(
      "/app/google-api?error=" +
        encodeURIComponent(`Token exchange failed: ${error.message}`)
    );
  }
};
