...
async function getSkuFwnMapping(
  invoiceSkus: string[],
  sheetsService: any,
    ...
    for (const row of sheetData.values) {
      if (row.length >= 4) {
        ...
        if (invoiceSku && skuFwn) {
          ...
          const normalizedInvoiceSku = invoiceSku
            .toString()
            .trim()
            .toUpperCase();
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    // Check if invoice is being processed (items might be empty during reprocessing)
    if (
      invoice.status === "PROCESSING" &&
      (!invoice.items || invoice.items.length === 0)
    ) {
      return new Response(
        "Invoice is currently being processed. Please wait for processing to complete and try again.",
        {
          status: 409, // Conflict - resource is being modified
          headers: {
    ...
    if (!invoice.items || invoice.items.length === 0) {
      return new Response(
        "Invoice has no items to export. The invoice may be empty or still processing.",
        {
          status: 400,
          headers: {
            "Content-Type": "text/plain",
    ...
    if (settings.serviceAccountConfig) {
      const { createServiceAccountServiceFromConfig } = await import(
      ...
      const { createOAuth2ServiceFromConfig } = await import(
        "../services/googleSheets"
      );
      ...
      return new Response("Google Sheets authentication not configured", {
        status: 500,
      });
    }

    // Get SKU FWN mapping from Google Sheets
    const invoiceSkus = invoice.items.map((item: any) =>
      item.sku.trim().toUpperCase()
    );
    const skuFwnMapping = await getSkuFwnMapping(
      invoiceSkus,
      sheetsService,
    ...
    return new Response(`Error generating Excel: ${error.message}`, {
      status: 500,
    });
  }
};
