...
async function main() {
  const pdfPath = join(process.cwd(), "samples", "Prolife.pdf");
  console.log(`Parsing Prolife invoice: ${pdfPath}`);

  const result = await parseInvoiceFromPdf(pdfPath, "Prolife", false);
