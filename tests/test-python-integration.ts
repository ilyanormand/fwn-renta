...
async function testPythonIntegration() {
  ...
  const pdfPath = join(process.cwd(), "samples", "Yamamoto.pdf");
  ...
  try {
    ...
    try {
      ...
      if (tableResult.tables && tableResult.tables.length > 0) {
        ...
        console.log(
          `  Shape: ${firstTable.shape ? `${firstTable.shape[0]}x${firstTable.shape[1]}` : "unknown"}`
        );
    ...
    try {
      ...
      if (result.success && result.data) {
        console.log(`\n📝 Invoice Metadata:`);
        console.log(
          `  Invoice Number: ${result.data.invoiceMetadata.invoiceNumber || "Not found"}`
        );
        console.log(
          `  Invoice Date: ${result.data.invoiceMetadata.invoiceDate || "Not found"}`
        );
        ...
        result.data.lineItems.slice(0, 5).forEach((item, index) => {
          ...
          console.log(
            `     Unit Price: €${item.unitPrice ? item.unitPrice.toFixed(2) : "N/A"}`
          );
          console.log(
        ...
        if (result.data.lineItems.length > 5) {
          console.log(
            `  ... and ${result.data.lineItems.length - 5} more items`
          );
        }
    ...
    try {
      const regularResult = await parseInvoiceFromPdf(
        pdfPath,
        "IAF Network",
        false
      );
