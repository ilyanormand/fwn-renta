  ...
  try {
    ...
    if (existsSync(envPath)) {
      ...
      for (const line of content.split(/\r?\n/)) {
        ...
        loadedVars++;
        console.log(
          `📝 Loaded: ${key} = ${key.includes("CONFIG") ? val.substring(0, 30) + "..." : val}`
        );
      }
...
async function testGoogleSheetsService() {
  ...
  if (!apiKey && !oauthConfig) {
    console.error(
      "❌ Missing both GOOGLE_SHEETS_API_KEY and GOOGLE_OAUTH_CONFIG"
    );
    console.log("💡 Add to .env file either:");
    console.log("GOOGLE_SHEETS_API_KEY=your_api_key_here");
    console.log("OR");
    console.log(
      'GOOGLE_OAUTH_CONFIG=\'{"web":{"client_id":"...","client_secret":"...","project_id":"..."}}\''
    );
    return;
  }
  ...
  try {
    ...
    if (columnData && columnData.values) {
      const nonEmptyRows = columnData.values.filter(
