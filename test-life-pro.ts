...
async function main() {
  const pdfPath = join(process.cwd(), "python/Life pro Facture.Pdf");
  console.log(`Parsing Life Pro invoice: ${pdfPath}`);

  const result = await parseInvoiceFromPdf(pdfPath, "Life pro", false);
